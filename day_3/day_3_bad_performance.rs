...
fn walk_right(x: i32, y: i32, steps: i32) -> [i32;2] {
    //println!("Walking U {} {}", x + steps, y);

    [x + steps, y]
}

fn walk_down(x: i32, y: i32, steps: i32) -> [i32;2] {
    //println!("Walking U {} {}", x, y - steps);

    [x, y - steps]
}

fn walk_left(x: i32, y: i32, steps: i32) -> [i32;2] {
    //println!("Walking L {} {}", x - steps, y);
...
fn main() {
    ...
    for w in wire_1_path.split(",") {
        let mut walk = String::from(w);
        let direction = walk.remove(0);
        let steps: i32 = walk.parse().unwrap();
